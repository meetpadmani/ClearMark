...
- Added the `@pilio/clearmark/video` SDK entrypoint for local video watermark removal with injectable processors and a Playwright-backed preview-page default.
...
- Shortened the extension name to `ClearMark` in the Manifest V3 metadata.
...
- Added isolated consumer smoke coverage that validates runtime subpath imports and rejects deep private imports from `@pilio/clearmark/src/...`.
