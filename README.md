...
# ClearMark — Lossless Watermark Removal Tool
...
> **🚀 Looking for the `Online ClearMark (Recommended)`? Try [geminiwatermarkremover.io](https://geminiwatermarkremover.io/)** — free, no install, works directly in your browser.
>
> 💡 Got watermarks that ClearMark can't handle? Try the general-purpose AI watermark remover: [pilio.ai/image-watermark-remover](https://pilio.ai/image-watermark-remover)
...
<p align="center">
  ...
  <a href="https://chromewebstore.google.com/detail/clearmark/cjlmnfcfnofnglkphbcdclbpimdjkmdf"><img src="https://img.shields.io/badge/🧩_Chrome_Extension-Install-blueviolet?style=for-the-badge" alt="Chrome Extension"></a>&nbsp;
  <a href="https://geminiwatermarkremover.io/userscript/clearmark.user.js"><img src="https://img.shields.io/badge/🐒_Userscript-Install-green?style=for-the-badge" alt="Userscript"></a>&nbsp;
...
<p align="center">
  <img src="https://count.getloli.com/@clearmark?name=clearmark&theme=minecraft&padding=7&offset=0&align=top&scale=1&pixelated=1&darkmode=auto" width="400">
</p>
...
> [!NOTE]
> **Note**: Disable any fingerprint defender extensions (e.g., Canvas Fingerprint Defender) to avoid processing errors. https://github.com/GargantuaX/clearmark/issues/3

## How to Remove Gemini Watermarks

### Online ClearMark (Recommended)
...
1. Open [ClearMark on the Chrome Web Store](https://chromewebstore.google.com/detail/clearmark/cjlmnfcfnofnglkphbcdclbpimdjkmdf).
...
Chrome Web Store installs update automatically. If the Web Store is not reachable in your network, the matching verified zip (`clearmark-extension-v*.zip`) remains available from [GitHub Releases](https://github.com/GargantuaX/clearmark/releases/latest) and the official website download entry for manual unpacked installation.

### Userscript

1. Install a userscript manager (e.g., Tampermonkey or Greasemonkey).
2. Open [clearmark.user.js](https://geminiwatermarkremover.io/userscript/clearmark.user.js).
...
- `skills/clearmark/` contains a packaged Skill that agents can discover and invoke.
- Install it with `skills.sh` using:

```bash
pnpm dlx skills add GargantuaX/clearmark --skill clearmark
```
...
```bash
node skills/clearmark/scripts/run.mjs remove <input> --output <file>
```

- See [`SKILL.md`](skills/clearmark/SKILL.md) for agent integration details.
...
```bash
pnpm dlx @pilio/clearmark remove <input> --output <file>
```
...
```javascript
import { removeWatermarkFromBuffer } from '@pilio/clearmark/node';
...
├── skills/
│   └── clearmark/    # Distributable agent skill bundle
├── src/
...
- `skills/clearmark/` provides a distributable Skill that stays on the CLI boundary instead of importing repository internals directly.
