...
const TEXT = {
    ...
    unsupportedFile: 'Please select a JPG, PNG, WebP image, or MP4/WebM/MOV video.',
    fileTooLarge: 'Images over 20MB are not processed here. Videos will go to the video preview page.',
    skippedLargeImages: 'Images over 20MB were skipped.',
    handoffVideo: 'Entering video processing flow...',
    progress: 'Progress',
    pending: 'Pending',
...
async function routeVideoFile(file) {
    try {
        ...
        hideLoading();
        console.error(error);
        setStatusMessage(error.message || 'Unable to enter video processing flow. Please open the video page and reselect the file.', 'warn');
    }
}
...
async function consumePendingImageHandoff() {
    ...
    try {
        ...
        setStatusMessage(error.message || 'Failed to read image handoff. Please reselect the file.', 'warn');
    }
}
    ...
    card.innerHTML = `
        <div class="batch-comparison">
            <div class="batch-pane original">
                <span class="batch-pane-label orig">Original</span>
                <img id="original-${item.id}" class="batch-image" draggable="false" alt="" />
            </div>
            <div class="batch-pane processed">
                <span class="batch-pane-label proc">Processed</span>
