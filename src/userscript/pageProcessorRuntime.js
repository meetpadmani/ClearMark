...
export async function installInjectedPageProcessorRuntime({
  ...
  try {
    ...
    logger?.info?.('[ClearMark] Page runtime inline injection did not register, retrying with blob script');
    ...
    logger?.warn?.('[ClearMark] Page runtime injection failed:', error);
    return null;
  }

  if (!targetWindow[PAGE_PROCESS_RUNTIME_FLAG]) {
    logger?.warn?.('[ClearMark] Page runtime injection did not register a bridge');
    return null;
  }
