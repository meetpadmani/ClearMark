...
export function createUserscriptProcessingRuntime({
  ...
  function disableInlineWorker(reason) {
    ...
    logger?.warn?.('[ClearMark] Disable worker path:', reason);
  ...
  function emitTiming(stage, payload = {}) {
    ...
    logger?.info?.(`[ClearMark] timing ${stage}`, payload);
  }
  ...
  async function processBlobWithBestPath(blob, options = {}) {
    ...
    if (workerClient) {
      try {
        ...
        logger?.warn?.('[ClearMark] Worker path failed, fallback to main thread:', error);
  ...
  const runtime = {
    async initialize() {
      ...
      try {
        ...
        logger?.log?.('[ClearMark] Worker acceleration enabled');
        return true;
      } catch (workerError) {
        workerClient?.dispose();
        workerClient = null;
        logger?.warn?.('[ClearMark] Worker initialization failed, using main thread:', workerError);
        return false;
      }
    },
    dispose(reason) {
