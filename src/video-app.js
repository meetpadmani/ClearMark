...
async function resolveExportDenoiseRuntime(denoiseBackend, runtimeProfile = resolveAllenkFdncnnRuntimeProfile()) {
    ...
    setStatus('Loading AI FDnCNN ONNX model, first load may take a moment...');
    ...
    return ({ progress = 0, step = 'detect', sampledFrames = 0, sampleCount = 0 } = {}) => {
        ...
        if (step === 'sample') {
            setStatus(sampleCount > 0
                ? `Processing Sampling frames检测水印：${sampledFrames}/${sampleCount}`
                : 'Processing Sampling frames检测水印...');
        } else if (step === 'score') {
            setStatus('Processing Matching watermarkCandidate，page will remain responsive...');
        }
    };
}
...
async function playComparison() {
    ...
    try {
        ...
        setStatus('Browser preventedPlay，please click againPlaybutton。', 'warn');
    } finally {
        updatePlaybackControls();
    }
}
...
function renderAutoPresetSummary(preset = null) {
    ...
    if (!preset) {
        els.autoPresetSummary.innerHTML = `
            ...
            <span>Automatically detects watermarks after selecting a video, and uses a local AI model to clean them on export.</span>
        `;
        return;
    }
...
function renderMetadata(metadata) {
    ...
    els.metadata.innerHTML = `
        <dl>
            ...
            <div><dt>Watermark Spec</dt><dd>${reference ? '1920x1080 Confirmed' : 'Inferred ratio, Experimental'}</dd></div>
        </dl>
    `;
}
...
function renderDetection(detection) {
    if (!detection) {
        els.detection.innerHTML = '<p class="muted">Detect or export to see results</p>';
        return;
    }
...
async function setFile(file) {
    ...
    if (fileKind !== 'video') {
        setStatus('Please select an image or video file. Videos are processed here, images go to the single image page.', 'warn');
        return;
    }
    ...
    setProgress(0, 'Ready');
    setStatus('Processing Reading videometadata...');
    updateButtons();
    ...
    try {
        ...
        console.error(error);
        setStatus(error.message || 'Reading videofailed', 'error');
    } finally {
        updateButtons();
    }
}
...
async function routeImageFile(file) {
    try {
        setStatus('Entering image processing flow...');
        await saveDebugFileHandoff(file, 'image');
        window.location.assign('./dev-preview.html?fileHandoff=1');
    } catch (error) {
        console.error(error);
        setStatus(error.message || 'Unable to enter image flow, please open image page and reselect file.', 'warn');
    }
}
...
async function runDetection() {
    ...
    setStatus('Processing Sampling framesDetecting bottom-right watermark...');
    ...
    try {
        ...
        if (preset.id === 'relocated-review') {
            setStatus('Detection complete，will use on export AI watermark removal。', result.detection.isConfident ? 'success' : 'warn');
        } else {
            setStatus(result.detection.isConfident ? 'Detection complete，will use on export AI watermark removal。' : 'Detection confidence low, but AI export can still be attempted.', result.detection.isConfident ? 'success' : 'warn');
        }
    } catch (error) {
        console.error(error);
...
async function runExport() {
    ...
    try {
        ...
        const audioNote = result.audioCopied
            ? `Audio retained：${result.audioCodec || 'unknown'}，${result.audioPacketCount || 0} packets。`
            : `Audio not retained：${result.audioSkipReason || 'unknown'}。`;
...
function applyRelocatedReviewPreset() {
    ...
    applyPresetToControls(preset);
    setStatus('Relocated Anchor preset applied：Canvas Footprint Polish、12Mbps、AllowLow confidence。This preset is for manual review，Not default strategy。', 'warn');
}
...
async function consumePendingVideoHandoff() {
    ...
    try {
        ...
        setStatus(error.message || 'Reading videoStash failed，Please reselect the file。', 'warn');
    }
}
