...
- userscript bundle in `dist/userscript/clearmark.user.js`
- package/sdk source and metadata from `package.json`, `src/core/`, and `src/sdk/`
- Chrome Web Store listing plus fallback package in `release/clearmark-extension-v<version>.zip`
...
- `dist/userscript/clearmark.user.js` is regenerated
...
- load the unpacked local Chrome extension from `dist/extension` and verify the popup toggle, Gemini online link, general watermark link, and GitHub feedback link; confirm the extension card is labeled `ClearMark Local`
- verify the live Chrome Web Store listing points to:
  `https://chromewebstore.google.com/detail/clearmark/cjlmnfcfnofnglkphbcdclbpimdjkmdf`
...
- create a GitHub Release from that tag and upload the built userscript from `dist/userscript/clearmark.user.js`
- upload `release/clearmark-extension-v<version>.zip`, its `.sha256.txt` file, and `latest-extension.json` to GitHub Release as the manual fallback package
...
- Check downstream projects for a direct package dependency before bumping them:
  `rg -n "@pilio/clearmark" --glob package.json --glob pnpm-lock.yaml`
- Do not treat task routes, processor keys, i18n copy, tests, or docs mentioning `clearmark` as evidence that the npm SDK must be bumped.
- Only update a downstream project when it directly depends on `@pilio/clearmark`.
...
gh release create v<version> \
  dist/userscript/clearmark.user.js \
  release/clearmark-extension-v<version>.zip \
  release/clearmark-extension-v<version>.zip.sha256.txt \
...
1. Run the website's userscript build/sync command.
   ...
   - It copies `dist/userscript/clearmark.user.js` to `public/userscript/clearmark.user.js`.
2. If the npm SDK was published for this release and the website still directly depends on it, update `@pilio/clearmark` to the released version and refresh `pnpm-lock.yaml`.
...
- confirm the GitHub Release latest userscript serves the latest bundle:
  `https://github.com/GargantuaX/clearmark/releases/latest/download/clearmark.user.js`
- confirm the official website serves the latest userscript bundle:
  `https://geminiwatermarkremover.io/userscript/clearmark.user.js`
