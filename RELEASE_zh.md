...
- 网站构建产物：`dist/`
- 油猴脚本产物：`dist/userscript/clearmark.user.js`
- `package.json`、`src/core/`、`src/sdk/` 对应的 package/sdk 源码与元数据
- Chrome Web Store 商店页，以及备用安装包：`release/clearmark-extension-v<version>.zip`
...
- `dist/userscript/clearmark.user.js` 已重新生成
...
- 验证预览图处理失败时页面原图仍保持可见
- 从 `dist/extension` 加载未打包本地 Chrome 插件，验证弹窗开关、Gemini 在线工具链接、通用去水印链接和 GitHub 反馈链接；确认扩展卡片显示为 `ClearMark Local`
...
- 基于该 tag 创建 GitHub Release，并上传 `dist/userscript/clearmark.user.js`
- 上传 `release/clearmark-extension-v<version>.zip`、对应 `.sha256.txt` 和 `latest-extension.json` 到 GitHub Release，作为手动安装备用包
...
- 不要把任务路由、processor key、i18n 文案、测试或文档中出现的 `clearmark` 当成必须升级 npm SDK 的证据。
- 只有下游项目直接依赖 `@pilio/clearmark` 时，才更新该依赖。
...
1. 在官网项目中运行 userscript 构建/同步命令。
   - 该命令会重新构建当前上游仓库。
   - 然后把 `dist/userscript/clearmark.user.js` 复制到 `public/userscript/clearmark.user.js`。
2. 如果本次已经发布 npm SDK，且官网仍直接依赖该 SDK，则把 `@pilio/clearmark` 更新到对应版本并刷新 `pnpm-lock.yaml`。
