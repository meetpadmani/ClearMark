...
const PACKAGE_BASE_NAME = 'clearmark-extension';
...
function createInstallDocs(version, { packageBaseName = PACKAGE_BASE_NAME } = {}) {
  const english = `# ClearMark Chrome Extension v${version}
...
`;
  ...
  const chinese = `# ClearMark Chrome 插件 v${version}
...
function removeStaleLocalReleaseArtifacts() {
  ...
  for (const entry of readdirSync(RELEASE_DIR, { withFileTypes: true })) {
    if (entry.name === 'latest-extension-local.json' || entry.name.startsWith('clearmark-extension-local-')) {
