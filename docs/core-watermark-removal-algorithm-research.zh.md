...
- userscript artifact：校验 `dist/userscript/clearmark.user.js` 的 `@version`、`@downloadURL`、`@updateURL` 和关键 request-layer runtime markers。
...
- Userscript artifact：`ready`
  - `dist/userscript/clearmark.user.js` 存在，`@version = 1.0.20`，与 `package.json` 一致。
