...
async function writeUserscriptArtifact(tempDir, version = '1.2.3') {
    const userscriptPath = path.join(tempDir, 'dist', 'userscript', 'clearmark.user.js');
...
test('createReleaseReadinessReport should allow scoped image RC while blocking video parity claims', async () => {
    ...
    const extensionFile = 'clearmark-extension-v1.2.3.zip';
    ...
    assert.match(report.overall.releaseEvidenceIndex.releasePackage.zipPath, /clearmark-extension-v1\.2\.3\.zip$/);
