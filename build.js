...
// @author       GargantuaX
// @license      MIT
// @downloadURL  https://github.com/GargantuaX/clearmark/releases/latest/download/clearmark.user.js
// @updateURL    https://github.com/GargantuaX/clearmark/releases/latest/download/clearmark.user.js
...
function createExtensionManifest({ profile = 'official' } = {}) {
  ...
  const displayName = isOfficial ? 'ClearMark' : 'ClearMark Local';
  const manifest = {
    manifest_version: 3,
    name: displayName,
...
async function serveStaticDevDist(rootDir = 'dist', defaultPort = 4173) {
  ...
  server.listen(port, '0.0.0.0', () => {
    console.log(`\n🔒 Secure Context: ${protocol}://0.0.0.0:${port} (Accessible via LAN IP)`);
    console.log(`🌐 Dev server running at ${protocol}://localhost:${port}`);
    console.log(`   ↳ internal debug harness: ${protocol}://localhost:${port}/ (dev-preview.html)`);
    console.log(`   ↳ public landing entry:   ${protocol}://localhost:${port}/index.html\n`);
  });
...
const userscriptCtx = await esbuild.context({
  ...
  format: 'iife',
  outfile: 'dist/userscript/clearmark.user.js',
