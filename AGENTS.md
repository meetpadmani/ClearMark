...
- Production userscript artifact: `dist/userscript/clearmark.user.js`
...
5. Install or reinstall the production userscript from the current local build server when validating the latest build.
   ...
   - A previously confirmed request-layer debugging session used `http://127.0.0.1:4317/userscript/clearmark.user.js`, but do not assume that is still current
...
- Current command behavior:
  - exits `0` when the installed userscript exactly matches the local `dist/userscript/clearmark.user.js`
...
- Manual fallback if needed:
  1. Compare the installed source against `dist/userscript/clearmark.user.js`
...
- If `[ClearMark] Worker acceleration enabled` appears, that only means startup was attempted.
- The worker is only considered usable if the startup handshake succeeds.
- If `[ClearMark] Worker initialization failed, using main thread: ...` appears, safe fallback has happened.
