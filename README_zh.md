...
<p align="center">
  ...
  <a href="https://chromewebstore.google.com/detail/clearmark/cjlmnfcfnofnglkphbcdclbpimdjkmdf"><img src="https://img.shields.io/badge/🧩_Chrome插件-安装-blueviolet?style=for-the-badge" alt="Chrome 插件"></a>&nbsp;
  <a href="https://geminiwatermarkremover.io/userscript/clearmark.user.js"><img src="https://img.shields.io/badge/🐒_油猴脚本-安装-green?style=for-the-badge" alt="油猴脚本"></a>&nbsp;
...
> [!NOTE]
> 另请注意：使用此工具需禁用 Canvas 指纹防护扩展（如 Canvas Fingerprint Defender），否则可能会导致处理结果错误。 https://github.com/GargantuaX/clearmark/issues/3
...
1. 打开 [Chrome Web Store 上的 ClearMark](https://chromewebstore.google.com/detail/clearmark/cjlmnfcfnofnglkphbcdclbpimdjkmdf)
...
通过 Chrome Web Store 安装后，插件会自动更新。如果你的网络无法稳定访问 Chrome Web Store，仍可从 [GitHub Releases](https://github.com/GargantuaX/clearmark/releases/latest) 或官网备用下载入口获取同版本已校验 zip（`clearmark-extension-v*.zip`），并按手动加载未打包插件的方式安装。

### 油猴脚本

1. 安装油猴插件（如 Tampermonkey 或 Greasemonkey）
2. 打开 [clearmark.user.js](https://geminiwatermarkremover.io/userscript/clearmark.user.js)
3. 脚本会自动安装到浏览器中
4. 打开 Gemini 对话页面
...
- `skills/clearmark/` 包含打包好的 Skill，AI agent 可自动发现并调用。
- 用 `skills.sh` 安装可执行：
...
- 详见 [`SKILL.md`](skills/clearmark/SKILL.md)。
...
├── skills/
│   └── clearmark/    # 可分发的 agent Skill bundle
├── src/
│   ├── assets/            # 校准资源与回归样本
│   ├── cli/               # CLI 参数解析与文件工作流
...
- `skills/clearmark/` 提供可分发的 Skill bundle，并且严格停留在 CLI 边界，不直接导入仓库内部实现。
